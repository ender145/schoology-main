...
(function(win) {
	...
	var tinymce = {
		...
		_init : function() {
			...
			// SGY-12411 -- Added IE11 detection and updated isGecko detection. Reused logic from tinyMCE source v4.2.1
...
tinymce.util.Quirks = function(editor) {
  ...
  // SGY-5281 patched
  // @see https://github.com/tinymce/tinymce/commit/113bd1ceaef1e1de5f2fdf54633d58424817182c
		...
		function fixInvalidChildren(nodes) {
			...
			for (ni = 0; ni < nodes.length; ni++) {
				...
				if (parent && parents.length > 1) {
					...
					// Check if the element is empty by looking through its contents and special treatment for <p><br /></p>
					parent = parents[0];
	...
	tinymce.create('tinymce.ui.KeyboardNavigation', {
		KeyboardNavigation: function(settings, dom) {
			...
			itemBlurred = function(evt) {
				dom.setAttrib(evt.target.id, 'tabindex', '0');
			};
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.ColorSplitButton:tinymce.ui.SplitButton', {
		...
		showMenu : function() {
			...
			p2 = DOM.getPos(e);
			const element = DOM.get(t.id + '_menu');

			if (isRtl) {
				DOM.setStyles(element, {
					left : p2.x - element.clientWidth + e.clientWidth,
	...
	tinymce.create('tinymce.Editor', {
		...
		render : function(nst) {
			...
			function loadScripts() {
              ...
              if (p === 'inlineLink') {
                var inlineLinkPluginURL = (typeof Drupal.settings.s_tinymce != 'undefined' && typeof Drupal.settings.s_tinymce.inlineLinkPluginURL != 'undefined') ? Drupal.settings.s_tinymce.inlineLinkPluginURL : false;
                if (inlineLinkPluginURL) {
                  PluginManager.load(p, inlineLinkPluginURL);
                }
                return;
              }

              if (p === 'promptBank') {
                var promptBankPluginURL = (typeof Drupal.settings.s_tinymce != 'undefined' && typeof Drupal.settings.s_tinymce.promptBankPluginURL != 'undefined') ? Drupal.settings.s_tinymce.promptBankPluginURL : false;
                if (promptBankPluginURL) {
                  PluginManager.load(p, promptBankPluginURL);
                }
                return;
              }

              if (p === 'accessibilityChecker') {
                var accessibilityCheckerPluginURL = (typeof Drupal.settings.s_tinymce != 'undefined' && typeof Drupal.settings.s_tinymce.accessibilityCheckerPluginURL != 'undefined') ? Drupal.settings.s_tinymce.accessibilityCheckerPluginURL : false;
                if (accessibilityCheckerPluginURL) {
                  PluginManager.load(p, accessibilityCheckerPluginURL);
                }
                return;
              }
		...
		focus : function(skip_focus) {
			...
			if (!skip_focus) {
				...
				if (tinymce.isGecko || contentEditable) {
					...
					if (body.setActive) {
            // In IE9+, attempting to setActive can cause a js error when content editable is not visible
            // similar issue that appeared on another WYSIWYG: https://github.com/xing/wysihtml5/issues/9
						try{
              body.setActive();
            }
            catch(e){
	...
	tinymce.EditorCommands = function(editor) {
		...
		addCommands({
			...
			'InsertUnorderedList,InsertOrderedList' : function(command) {
				var listElm, listParent, directionParent;
				...
				if (listElm) {
					directionParent = dom.getAttrib(listElm.parentNode, "dir");
					dom.setAttrib(listElm, "dir", directionParent);
	...
	tinymce.Formatter = function(ed) {
		...
		function apply(name, vars, node) {
			...
			function applyRngStyle(rng, bookmark, node_specific) {
                ...
                // don't merge the span tag of a bookmark node, which will effectively delete the bookmark.
	...
	tinymce.EnterKey = function(editor) {
		...
		function handleEnterKey(evt) {
			...
			function handleEmptyListItem() {
				...
				var directionParent = dom.getAttrib(containerBlock, "dir");
				//set dir attribute to newBlock node
				dom.setAttrib(newBlock, "dir", directionParent);
				undoManager.add();
			};
