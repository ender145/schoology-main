...
(function() {
	...
	tinymce.create('tinymce.plugins.MediaPlugin', {
		init : function(ed, url) {
			...
			ed.addCommand('mceMedia', function() {
				...
				ed.windowManager.open({
					file : url + '/media.htm',
					width : 476 + parseInt(ed.getLang('media.delta_width', 0)),
					height : 541 + parseInt(ed.getLang('media.delta_height', 0)),
					inline : 1
				}, {
					plugin_url : url,
					data : data
		...
		objectToImg : function(node) {
			...
			if (object && !type)
				type = (lookup[(object.attr('clsid') || '').toLowerCase()] || lookup[(object.attr('classid') || '').toLowerCase()] || lookup[(object.attr('type') || '').toLowerCase()] || {}).name;
