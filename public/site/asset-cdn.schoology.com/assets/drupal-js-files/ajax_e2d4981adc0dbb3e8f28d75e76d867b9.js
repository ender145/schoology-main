  ...
  if (options.action === 'notify') {
    ...
    // Status, Error, Message
      ...
      if (options.type === 'error') {
        // So Firefox (and other AT) announce the full error text, not just "alert":
        // put the message text in a single element as the first child and make the
        // region atomic so the whole content is read.
        errBox.attr('role', 'alert');
        errBox.attr('aria-atomic', 'true');
        var announcementParts = [];
        for (i = 0; i < options.messages.length; i++) {
          announcementParts.push(options.messages[i].value);
        }
        var announcementText = announcementParts.join(' ');
        var $announcement = $('<span>').addClass('visually-hidden').text(announcementText);
        log.attr('aria-hidden', 'true');
        errBox.empty().append($announcement).append(log);
      } else {
        errBox.html(log);
      }
    }
