	...
	tinymce.create('tinymce.themes.AdvancedTheme', {
		...
		init : function(ed, url) {
			...
			t.settings = s = extend({
				...
				theme_advanced_blockformats : "p,h1,h2,h3,h4,pre,blockquote",
...
*/
		...
		_addControls : function(v, tb) {
			...
			each(explode(v), function(n) {
				...
				if (n == 'tablecontrols') {
					each(["table","|","row_before","row_after","delete_row","|","col_before","col_after","delete_col","|","split_cells","merge_cells"], function(n) {
    ...
    _makeUrl: function(filename){
      ...
      if(this.settings.cache_string){
        url += (url.indexOf('?') >= 0 ? '&' : '?') + 'cache=' + this.settings.cache_string;
      }
      return url;
    },
        ...
        url : this._makeUrl('charmap.htm'),
				width : 613 + parseInt(ed.getLang('advanced.charmap_delta_width', 0)),
				height : 320 + parseInt(ed.getLang('advanced.charmap_delta_height', 0)),
				inline : true
			}, {
				theme_url : this.url
			});
        ...
        url : this._makeUrl('color_picker.htm'),
				width : 360 + parseInt(ed.getLang('advanced.colorpicker_delta_width', 0)),
        ...
        url : this._makeUrl('image.htm'),
				width : 400 + parseInt(ed.getLang('advanced.image_delta_width', 0)),
				height : 250 + parseInt(ed.getLang('advanced.image_delta_height', 0)),
				inline : true
			}, {
				theme_url : this.url
			});
		...
		_mceLink : function(ui, val) {
			...
			ed.windowManager.open({
				...
				width : 400 + parseInt(ed.getLang('advanced.link_delta_width', 0)),
				height : 160 + parseInt(ed.getLang('advanced.link_delta_height', 0)),
				inline : true
			}, {
				theme_url : this.url
			});
